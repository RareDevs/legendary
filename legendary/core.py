...
class LegendaryCore:
    ...
    def prepare_download(self, app_name: str, base_path: str = '',
        ...
        if repair:
            ...
            if not os.path.exists(repair_file):
                ...
                try:
                    self.verify_game(app_name, callback=None, repair_mode=True, repair_online=repair_use_latest)
                except ValueError:
                    raise RuntimeError('To repair a game with a missing manifest you must run the command with '
        ...
        return dlm, anlres, igame, game, repair, repair_file, res
    ...
    def verify_game(self, app_name: str, callback: Callable[[int, int, float, float], None] = print, repair_mode=False, repair_online=False):
        logger = self.log
        if not self.is_installed(app_name):
            logger.error(f'Game "{app_name}" is not installed')
            return

        logger.info(f'Loading installed manifest for "{app_name}"')
        igame = self.get_installed_game(app_name)
        ...
        if manifest_data is None:
            if repair_mode:
                ...
                game = self.get_game(app_name, platform=igame.platform)
                manifest_data, _ = self.get_cdn_manifest(game, igame.platform)
            else:
                raise RuntimeError(f'Manifest appears to be missing! To repair, run "legendary repair '
                                f'{app_name} --repair-and-update", this will however redownload all files '
        ...
        if config_tags := self.lgd.config.get(app_name, 'install_tags', fallback=None):
        ...
        for result, path, result_hash, bytes_read in validate_files(igame.install_path, file_list):
            ...
            if callback:
                callback(num, total, percentage, speed)
    ...
    def prepare_overlay_install(self, path=None,
                                status_q: Queue = None, max_workers=0, update_interval=1.0,
                                dl_timeout=10, max_shared_memory=1024 ** 3, force: bool = False):
        # start anoymous session for update check if we're not logged in yet
        if not self.logged_in:
            self.egs.start_session(client_credentials=True)
        ...
        dlm = DLManager(path, base_urls[0],
                        status_q=status_q, max_workers=max_workers, update_interval=update_interval,
