...
class LegendaryCLI:
    ...
    def install_game(self, args):
        ...
        dlm, analysis, igame, game, repair, repair_file, res = self.core.prepare_download(app_name=args.app_name, base_path=args.base_path,
        ...
        if not analysis.dl_size:
            # lk: Moved to clean_post_install
        ...
        try:
            ...
            while dlm.is_alive():
                try:
                    ...
                    logger.info(f' + Download\t- {status.download_compressed_speed / 1024 / 1024:.02f} MiB/s (raw) '
                                f'/ {status.download_speed / 1024 / 1024:.02f} MiB/s (decompressed)')
    ...
    def verify_game(self, args, print_command=True, repair_mode=False, repair_online=False):
        args.app_name = self._resolve_aliases(args.app_name)
        def report_progress(num, total, percentage, speed):
            stdout.write(f'Verification progress: {num}/{total} ({percentage:.01f}%) [{speed:.1f} MiB/s]\t\n')
            stdout.flush()
        try:
            self.core.verify_game(app_name=args.app_name, callback=report_progress, repair_mode=repair_mode, repair_online=repair_online)
